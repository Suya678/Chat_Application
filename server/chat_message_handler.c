...
void process_client_message(Client *client, Worker_Thread *thread_context) {
    ...
    if (bytes_received <= 0) {
        ...
        LOG_INFO("Thread (%lu): Client fd %d disconnected during receive\n",
                 thread_context->id, client->client_fd);
        handle_client_disconnection(client, thread_context);
        return;
    }

    read_buffer[bytes_received] = '\0';
    LOG_INFO("Thread (%lu): Received %zd bytes from client fd %d: %s\n",
             thread_context->id, bytes_received, client->client_fd, read_buffer);
    ...
    while (msg_term != NULL) {
        *msg_term = '\0';
        ...
        LOG_INFO("Thread (%lu): Processing complete message from client fd %d: %s\n",
                 thread_context->id, client->client_fd, client->current_msg);
    ...
    if (*temp != '\0') {
        strcat(client->current_msg, temp);
        LOG_INFO("Thread (%lu): Stored partial message from client fd %d: %s\n",
                 thread_context->id, client->client_fd, client->current_msg);
    }
}
        ...
        if (bytes == -1) {
            if (errno != EAGAIN && errno != EWOULDBLOCK) {
                LOG_ERROR("Failed to send message to client fd %d: %s. Message: %s\n",
                          client_fd, strerror(errno), message);
                break;
            }
...
static void route_client_command(Client *client,
    ...
    LOG_INFO("Thread (%lu): Routing command '0x%x' from client fd %d in (state: %d)\n",
             thread_context->id, client->current_msg[0], client->client_fd, client->state);

    if (client->current_msg[0] == CMD_EXIT) {
        LOG_INFO("Thread (%lu): Client fd %d requested exit\n",
                 thread_context->id, client->client_fd);
        handle_client_disconnection(client, thread_context);
        return;
    }
...
static void cleanup_client(Client *client, Worker_Thread *thread_context) {
    LOG_INFO("Thread (%lu) Cleaning up client %s (fd %d) resources\n",
             thread_context->id, client->name, client->client_fd);
    if (epoll_ctl(thread_context->epoll_fd, EPOLL_CTL_DEL, client->client_fd,
                  NULL) == -1) {
        LOG_ERROR("Thread (%lu): Failed to remove client fd %d from epoll: %s\n",
                  thread_context->id, client->client_fd, strerror(errno));
    }
    ...
    LOG_INFO("Thread (%lu): Client cleaned up and decremented client count to %d\n",
             thread_context->id, thread_context->num_of_clients);
...
static void leave_room(Client *client, int room_index) {
    ...
    LOG_INFO("Removed client %s (fd %d) from room %d, %d clients remaining\n",
             client->name, room_index, SERVER_ROOMS[room_index].num_clients);
    ...
    if (SERVER_ROOMS[room_index].num_clients == 0) {
        ...
        memset(SERVER_ROOMS[room_index].room_name, 0,
               sizeof(SERVER_ROOMS[room_index].room_name));
        memset(SERVER_ROOMS[room_index].clients, 0,
               sizeof(SERVER_ROOMS[room_index].clients));
...
static void join_chat_room(Client *client) {
    ...
    if (SERVER_ROOMS[room_index].num_clients == MAX_CLIENTS_ROOM) {
        LOG_ERROR("Client %s (fd %d) attempted to join a full room - %d: %s \n",
                  client->name, client->client_fd, SERVER_ROOMS[room_index].room_name, room_index);
...
static void create_chat_room(Client *client) {
    ...
    if (strlen(room_name) > MAX_ROOM_NAME_LEN) {
        LOG_ERROR("Client %s (fd %d) provided invalid room name length: %zu\n",
                  client->name, client->client_fd, strlen(room_name));
...
static void handle_in_chat_room(Client *client) {
    ...
    if (command == CMD_ROOM_MESSAGE_SEND) {
        ...
        LOG_INFO("Client %s (fd %d) sending message in room %d: %s\n",
                 client->name, client->client_fd, room_index, msg);
        ...
        LOG_INFO("Client %s (fd %d) leaving room %d\n", client->client_fd, client->name, room_index);
