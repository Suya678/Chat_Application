...
static void process_epoll_events(struct epoll_event event_queue[], int event_count,
...
static int setup_new_user(Worker_Thread *thread_data, int client_fd) {
	...
	LOG_ERROR("Failed to setup new user -Race condition: received client fd %d when "
			  "already at capacity\n",
			  thread_data->id, client_fd);


	if (close(client_fd) == -1) {
		LOG_ERROR("Failed to close client fd %d: %s for \n", thread_data->id, client_fd,
				  strerror(errno));
	};
	return -1;
}
...
static void process_epoll_events(struct epoll_event event_queue[], int event_count,
	...
	for (int i = 0; i < event_count; i++) {
		...
		if (user == NULL) {
			LOG_ERROR("Could not find client struct for fd %d\n", event_queue[i].data.fd);
			continue;
		}
...
void *process_client_connections(void *worker) {
	...
	while (1) {
		int event_count =
			epoll_wait(thread_context->epoll_fd, event_queue, MAX_CLIENTS_PER_THREAD + 1, -1);
...
static void process_new_client(Worker_Thread *thread_context) {
	...
	if (read(thread_context->notification_fd, &value, sizeof(uint64_t)) == -1) {
		...
		LOG_ERROR("Failed to read from eventfd %d: %s\n", thread_context->notification_fd,
				  strerror(errno));
		return;
	}
	sem_post(&thread_context->sem);
	...
	if (register_with_epoll(thread_context->epoll_fd, client_fd) == false) {
		...
		thread_context->num_of_clients--;
		pthread_mutex_unlock(&thread_context->num_of_clients_lock);
		LOG_ERROR("Failed to register client fd %d with epoll, closing connection\n", client_fd);
		if (close(client_fd) == -1) {
