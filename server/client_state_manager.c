...
void send_message_to_client(const int client_fd, const char cmd_type, const char *message)
{
    ...
    while (sent < length)
    {
        ...
        if (bytes == -1)
        {
            if (errno != EAGAIN && errno != EWOULDBLOCK)
            {
                LOG_SERVER_ERROR("Failed to send message to client fd %d: %s. Message: %s\n", client_fd,
 ...
 * message field
 * @return true if command is valid for client's current state, false otherwise
 */
static bool command_valid_for_state(Client *client)
{
    ...
    else if (client->state == IN_CHAT_LOBBY && (command != CMD_ROOM_CREATE_REQUEST &&
                                                command != CMD_ROOM_JOIN_REQUEST && command != CMD_ROOM_LIST_REQUEST))
    {
...
static void handle_awaiting_username(Client *client)
{
    ...
    if (username_length > MAX_USERNAME_LEN)
    {
        ...
        send_message_to_client(client->client_fd, ERR_USERNAME_LENGTH, "User name too long, must be less than 32\n");
        return;
    }
